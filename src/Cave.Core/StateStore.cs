    ...
    public CaveState Load()
    {
        ...
        foreach (var path in new[] { StatePath, StatePath + ".bak" })
        {
            try
            {
                ...
                if (state.Version is not (1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9)) throw new NotSupportedException($"Save version {state.Version} is not supported. Your save has not been changed.");
                if (state.Agents is null || state.Chests is null || state.Links is null || state.Settings is null || state.Decorations is null || state.SeenDesktop is null
